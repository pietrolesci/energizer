...
class EntropyStrategy(AccumulatorStrategy):
    ...
    $$
        x_H = \underset{x \in D_{pool}}{\arg\max} \;
        − \sum_{k=1}^K \; p_\theta(y_k \mid x) \; \log p_\theta(y_k  \mid x)
    $$
...
class MarginStrategy(AccumulatorStrategy):
    ...
    Intuitively, if the classifiers is certain about a prediction, it would assign the majority of
    the probability mass to a specific class.

    It implements the entropy calculation using the
    [`margin_confidence`][energizer.acquisition_functions.margin_confidence]
    function. Look the for more details on how it is implemented.
    """
